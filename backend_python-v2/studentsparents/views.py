...
class StudentsParentsView(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='student-by-parent/(?P<parent_idno>[^/.]+)')
    def student_by_parent(self, request, parent_idno=None):
        studentsparents_records = StudentsParents.objects.filter(parentID__parentIdno=parent_idno).select_related(
            'studentID', 'parentID')
        ...
        if studentsparents_records.exists():
            ...
            response = {
                "message": "No records found for the provided parent national number",
                "status_code": 404,
                "data": []
            }

        return Response(response,
                        status=status.HTTP_200_OK if studentsparents_records.exists() else status.HTTP_404_NOT_FOUND)
